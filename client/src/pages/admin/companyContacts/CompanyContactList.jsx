  ...
  useEffect(() => {
    ...
    async function loadContacts() {
      try {
        ...
        if (mounted) {
          setError(err?.message || "企業お問い合わせの読み込みに失敗しました。");
        }
      } finally {
  ...
  const filteredContacts = useMemo(() => {
    ...
    return contacts
      .filter((contact) => {
        ...
        const companyName = String(contact.company?.companyName || "").toLowerCase();
      ...
      .sort(sortByNewest);
  }, [contacts, searchFilter, dateFilter]);
    ...
    <div className="min-h-[calc(100vh-3rem)] rounded-2xl bg-slate-100 shadow-sm">
      <div className="flex items-center justify-between rounded-t-2xl bg-slate-900 px-6 py-4">
        <h1 className="text-lg font-semibold text-white">企業お問い合わせ一覧</h1>
        <LogoutButton />
      </div>
      ...
      <div className="px-6 py-6">
        ...
        <div className="overflow-hidden rounded-xl bg-white shadow-sm">
          {loading ? (
            <div className="p-6 text-sm text-slate-600">企業お問い合わせを読み込み中...</div>
          ) : error ? (
            <div className="p-6 text-sm text-red-600">{error}</div>
          ) : filteredContacts.length === 0 ? (
            <div className="p-6 text-sm text-slate-600">企業お問い合わせが見つかりません。</div>
          ) : (
                  ...
                  {filteredContacts.map((contact, index) => (
                    <tr key={contact.id} className="hover:bg-slate-50">
                      ...
                      <td className="border-b border-slate-100 px-4 py-3 text-sm text-slate-700">
                        ...
                        <p className="mt-1 text-xs text-slate-500">{contact.phoneNumber || "-"}</p>
                      </td>
                      <td className="border-b border-slate-100 px-4 py-3 text-sm text-slate-700">
                        {formatCompanyPosition(contact.company?.position)}
                      </td>
                      <td className="border-b border-slate-100 px-4 py-3 text-sm text-slate-700">
                        {formatCompanyInquiryType(contact.company?.inquiryType)}
                      </td>
                      <td className="border-b border-slate-100 px-4 py-3 text-sm text-slate-700">
                      ...
                      <td className="border-b border-slate-100 px-4 py-3 text-sm text-slate-700">
                        <div className="flex items-center gap-2">
                          <Link
                            to={`/admin/company-contacts/${contact.id}`}
