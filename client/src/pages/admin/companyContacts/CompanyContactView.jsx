...
function formatCompanyPosition(value) {
  const positionMap = {
    MANAGER: "管理者",
    SECTION_CHIEF_MANAGER: "課長",
    CHIEF_SECTION_CHIEF: "部長",
    GENERAL_EMPLOYEE: "一般社員",
  };

  if (!value) return "-";
  return positionMap[value] || formatEnumLabel(value);
}

function formatCompanyInquiryType(value) {
...
function normalizeError(err) {
  ...
  return message || "企業お問い合わせの読み込みに失敗しました。";
}
...
export default function CompanyContactView() {
      ...
      try {
        ...
        if (mounted) {
          if (payload?.type !== "COMPANY") {
            setError("このデータは企業お問い合わせではありません。");
            setContact(null);
          } else {
            setContact(payload);
          }
  ...
  const pageTitle = useMemo(() => {
    if (!contact) return "企業お問い合わせ詳細";
    return contact.company?.companyName || getContactName(contact);
  }, [contact]);
    ...
    <div className="min-h-[calc(100vh-3rem)] rounded-2xl bg-slate-100 shadow-sm">
      ...
      <div className="px-6 py-6">
        ...
        <div className="overflow-hidden rounded-xl bg-white p-5 shadow-sm">
          {loading ? (
            <p className="text-sm text-slate-600">企業お問い合わせを読み込み中...</p>
          ) : error ? (
            ...
            <div className="space-y-4">
              <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                ...
                <ContactInfoRow label="会社名" value={contact.company?.companyName} />
                <ContactInfoRow label="担当者" value={getContactName(contact)} />
                <ContactInfoRow label="メールアドレス" value={contact.email} />
                <ContactInfoRow label="電話番号" value={contact.phoneNumber} />
                <ContactInfoRow label="郵便番号" value={contact.postalCode} />
                <ContactInfoRow label="住所" value={contact.address} />
                <ContactInfoRow label="役職" value={formatCompanyPosition(contact.company?.position)} />
                <ContactInfoRow
                  label="お問い合わせ種別"
                  value={formatCompanyInquiryType(contact.company?.inquiryType)}
                />
